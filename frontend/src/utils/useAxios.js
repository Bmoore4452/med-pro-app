...
import { API_BASE_URL } from './constants'; // Assuming you have a constants file for API_BASE_URL
...
useAxios.interceptors.request.use(
    async (config) => {
        ...
        if (isAccessTokenExpired(accessToken)) {
            ...
            console.log(
                '[useAxios] Access token expired, attempting refresh...'
            );
            if (!accessToken || !refreshToken) {
                console.warn('No tokens found — skipping auth header.');
                return config;
            }
...
useAxios.interceptors.response.use(
    ...
    (error) => {
        ...
        if (error.response && error.response.status === 401) {
            ...
            console.error(
                'Unauthorized access - you may need to log in:',
                error
            );
        }
